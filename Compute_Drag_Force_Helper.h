...
class Compute_Drag_Force_Helper
{
  ...
  public:
    Compute_Drag_Force_Helper(Hierarchy& hierarchy,const std::pair<const uint64_t*,unsigned>& blocks,
        T Struct_type::* drag_force_channel, T Struct_type::* alpha_channel, Channel_Vector& rel_face_velocity_channels,
        const int& axis)
    { Run(hierarchy,blocks,drag_force_channel,alpha_channel,rel_face_velocity_channels,axis);}
        ...
        const int& axis) const
    {
        ...
        auto vx_data=hierarchy.Allocator(level).template Get_Const_Array<Struct_type,T>(rel_face_velocity_channels(0));
        auto vy_data=hierarchy.Allocator(level).template Get_Const_Array<Struct_type,T>(rel_face_velocity_channels(1));
        auto v_axis_data=hierarchy.Allocator(level).template Get_Array<Struct_type,T>(rel_face_velocity_channels(axis));
        auto force_data=hierarchy.Allocator(level).template Get_Array<Struct_type,T>(drag_force_channel);
        ...
        auto compute_drag_force_helper=[&](uint64_t offset)
        {
            ...
            for(unsigned e=0;e<Flag_array_mask::elements_per_block;++e,offset+=sizeof(Flags_type)){
                    ...
                    if(flags(offset)&face_valid_mask) { T v_axis_value=v_axis_data(offset); T v_norm=std::sqrt(Nova_Utilities::Sqr(vx_data(offset))+Nova_Utilities::Sqr(vy_data(offset)));
                        force_data(offset)=std::pow((T).5,gamma)*alpha_data(offset)*rho*std::pow(mu,gamma)*std::pow(D,-(1+gamma))*std::pow(v_norm,1-gamma)*v_axis_value;
                    }
                range_iterator.Next();}
        };
        ...
        const int& axis) const
    {
        ...
        auto vx_data=hierarchy.Allocator(level).template Get_Const_Array<Struct_type,T>(rel_face_velocity_channels(0));
        auto vy_data=hierarchy.Allocator(level).template Get_Const_Array<Struct_type,T>(rel_face_velocity_channels(1));
        auto vz_data=hierarchy.Allocator(level).template Get_Const_Array<Struct_type,T>(rel_face_velocity_channels(2));
        auto v_axis_data=hierarchy.Allocator(level).template Get_Array<Struct_type,T>(rel_face_velocity_channels(axis));
        auto force_data=hierarchy.Allocator(level).template Get_Array<Struct_type,T>(drag_force_channel);
        auto alpha_data=hierarchy.Allocator(level).template Get_Const_Array<Struct_type,T>(alpha_channel);
